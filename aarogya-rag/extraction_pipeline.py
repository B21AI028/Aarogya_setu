...
#     """Extract JSON from PDF using Gemini (as in your script)."""
#     loader = PyPDFLoader(pdf_path)
#     pages = loader.load()
#     full_text = "\n\n".join([page.page_content for page in pages])
...
#     # Parse JSON (robust as in your script)
#     if "```json" in response_text:
#         start = response_text.find("```json") + 7
#         end = response_text.find("```", start)
#         json_text = response_text[start:end].strip()
...
# # Run the pipeline
# if __name__ == "__main__":
#     pdf_path = "Pragati_1.pdf"  # Your local PDF
#     extracted, full_text = extract_from_pdf(pdf_path)
#     summary = generate_summary(full_text, extracted["overall_interpretation"])
#     chunks = chunk_summary(summary)
#     store_to_db(extracted, full_text, chunks)